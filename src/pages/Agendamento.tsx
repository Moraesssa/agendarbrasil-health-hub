  ...
  if (isErrorSpecialties) {
    return (
      <div className="text-red-500">
        Erro ao carregar especialidades: {(errorSpecialties as Error).message}
      </div>
    );
  }
  if (isErrorDoctors) {
    return (
      <div className="text-red-500">
        Erro ao carregar médicos: {(errorDoctors as Error).message}
      </div>
    );
  }
  if (isErrorTimeSlots) {
    return (
      <div className="text-red-500">
        Erro ao carregar horários: {(errorTimeSlots as Error).message}
      </div>
    );
  }
