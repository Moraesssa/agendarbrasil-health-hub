
import { useState } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { SidebarProvider, SidebarInset, SidebarTrigger } from "@/components/ui/sidebar";
import { AppSidebar } from "@/components/AppSidebar";
import { Users, Search, Calendar, Phone, Mail, MapPin, Clock, Plus } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

const PacientesMedico = () => {
  const { toast } = useToast();
  const [searchTerm, setSearchTerm] = useState("");

  // Dados de exemplo - em produção viriam do banco de dados
  const pacientes = [
    {
      id: 1,
      nome: "Maria Silva",
      idade: 45,
      telefone: "(11) 99999-9999",
      email: "maria@email.com",
      endereco: "São Paulo, SP",
      ultimaConsulta: "2024-01-15",
      proximaConsulta: "2024-02-15",
      status: "Ativo"
    },
    {
      id: 2,
      nome: "João Santos",
      idade: 38,
      telefone: "(11) 98888-8888",
      email: "joao@email.com",
      endereco: "São Paulo, SP",
      ultimaConsulta: "2024-01-10",
      proximaConsulta: null,
      status: "Ativo"
    },
    {
      id: 3,
      nome: "Ana Costa",
      idade: 52,
      telefone: "(11) 97777-7777",
      email: "ana@email.com",
      endereco: "São Paulo, SP",
      ultimaConsulta: "2023-12-20",
      proximaConsulta: "2024-02-20",
      status: "Inativo"
    }
  ];

  const pacientesFiltrados = pacientes.filter(paciente =>
    paciente.nome.toLowerCase().includes(searchTerm.toLowerCase()) ||
    paciente.email.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleNovaConsulta = (pacienteId: number, nomePaciente: string) => {
    toast({
      title: "Agendar Nova Consulta",
      description: `Preparando agendamento para ${nomePaciente}`,
    });
  };

  const handleVerHistorico = (pacienteId: number, nomePaciente: string) => {
    toast({
      title: "Histórico do Paciente",
      description: `Visualizando histórico de ${nomePaciente}`,
    });
  };

  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-gradient-to-br from-slate-50 via-blue-50 to-green-50">
        <AppSidebar />
        <SidebarInset className="flex-1">
          <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-2 border-b border-blue-100/50 bg-white/95 backdrop-blur-md shadow-sm px-6">
            <SidebarTrigger className="text-blue-600 hover:bg-blue-50 transition-colors" />
            <div className="flex-1">
              <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-800 via-blue-600 to-green-600 bg-clip-text text-transparent">
                Meus Pacientes
              </h1>
              <p className="text-sm text-gray-600">Gerencie seus pacientes e consultas</p>
            </div>
            <Button className="bg-blue-500 hover:bg-blue-600">
              <Plus className="w-4 h-4 mr-2" />
              Novo Paciente
            </Button>
          </header>

          <main className="flex-1 overflow-auto">
            <div className="container max-w-7xl mx-auto p-6 space-y-6">
              {/* Estatísticas */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <Card className="shadow-lg">
                  <CardContent className="p-4 text-center">
                    <Users className="w-8 h-8 text-blue-600 mx-auto mb-2" />
                    <h3 className="text-2xl font-bold text-gray-900">{pacientes.length}</h3>
                    <p className="text-sm text-gray-600">Total de Pacientes</p>
                  </CardContent>
                </Card>
                <Card className="shadow-lg">
                  <CardContent className="p-4 text-center">
                    <Calendar className="w-8 h-8 text-green-600 mx-auto mb-2" />
                    <h3 className="text-2xl font-bold text-gray-900">
                      {pacientes.filter(p => p.proximaConsulta).length}
                    </h3>
                    <p className="text-sm text-gray-600">Consultas Agendadas</p>
                  </CardContent>
                </Card>
                <Card className="shadow-lg">
                  <CardContent className="p-4 text-center">
                    <Clock className="w-8 h-8 text-purple-600 mx-auto mb-2" />
                    <h3 className="text-2xl font-bold text-gray-900">
                      {pacientes.filter(p => p.status === "Ativo").length}
                    </h3>
                    <p className="text-sm text-gray-600">Pacientes Ativos</p>
                  </CardContent>
                </Card>
                <Card className="shadow-lg">
                  <CardContent className="p-4 text-center">
                    <Users className="w-8 h-8 text-orange-600 mx-auto mb-2" />
                    <h3 className="text-2xl font-bold text-gray-900">15</h3>
                    <p className="text-sm text-gray-600">Novos este Mês</p>
                  </CardContent>
                </Card>
              </div>

              {/* Busca */}
              <Card className="shadow-lg">
                <CardContent className="p-4">
                  <div className="relative">
                    <Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
                    <Input
                      placeholder="Buscar pacientes por nome ou email..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="pl-10"
                    />
                  </div>
                </CardContent>
              </Card>

              {/* Lista de Pacientes */}
              <div className="grid gap-4">
                {pacientesFiltrados.map((paciente) => (
                  <Card key={paciente.id} className="shadow-lg hover:shadow-xl transition-shadow">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-green-500 rounded-full flex items-center justify-center">
                            <span className="text-white font-bold">
                              {paciente.nome.split(' ').map(n => n[0]).join('')}
                            </span>
                          </div>
                          <div>
                            <h3 className="text-xl font-semibold text-gray-900">{paciente.nome}</h3>
                            <p className="text-gray-600">{paciente.idade} anos</p>
                            <Badge 
                              variant={paciente.status === "Ativo" ? "default" : "secondary"}
                              className={paciente.status === "Ativo" ? "bg-green-100 text-green-700" : ""}
                            >
                              {paciente.status}
                            </Badge>
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleVerHistorico(paciente.id, paciente.nome)}
                          >
                            Ver Histórico
                          </Button>
                          <Button
                            size="sm"
                            onClick={() => handleNovaConsulta(paciente.id, paciente.nome)}
                          >
                            Nova Consulta
                          </Button>
                        </div>
                      </div>

                      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          <Phone className="w-4 h-4 text-gray-400" />
                          <span>{paciente.telefone}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Mail className="w-4 h-4 text-gray-400" />
                          <span>{paciente.email}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <MapPin className="w-4 h-4 text-gray-400" />
                          <span>{paciente.endereco}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 h-4 text-gray-400" />
                          <span>
                            {paciente.proximaConsulta 
                              ? `Próxima: ${new Date(paciente.proximaConsulta).toLocaleDateString('pt-BR')}`
                              : "Sem consulta agendada"
                            }
                          </span>
                        </div>
                      </div>

                      <div className="mt-4 pt-4 border-t border-gray-100">
                        <p className="text-sm text-gray-600">
                          Última consulta: {new Date(paciente.ultimaConsulta).toLocaleDateString('pt-BR')}
                        </p>
                      </div>
                    </CardContent>
                  </Card>
                ))}
              </div>

              {pacientesFiltrados.length === 0 && (
                <Card className="shadow-lg">
                  <CardContent className="p-8 text-center">
                    <Users className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Nenhum paciente encontrado</h3>
                    <p className="text-gray-600">
                      {searchTerm ? "Tente ajustar os termos de busca" : "Adicione seu primeiro paciente"}
                    </p>
                  </CardContent>
                </Card>
              )}
            </div>
          </main>
        </SidebarInset>
      </div>
    </SidebarProvider>
  );
};

export default PacientesMedico;
