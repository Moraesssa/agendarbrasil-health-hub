...
interface Doctor {
  ...
  configuracoes: any; // Adicionado para horários
}
...
export const useAppointmentScheduling = () => {
  ...
  const loadDoctors = useCallback(async (specialty: string, state: string, city: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from('medicos')
        .select(`id, user_id, especialidades, configuracoes, profiles!medicos_user_id_fkey(display_name)`)
  ...
  useEffect(() => { if (selectedState) loadAvailableCities(selectedState); setSelectedCity(''); }, [selectedState, loadAvailableCities]);
  useEffect(() => { if (selectedSpecialty && selectedState && selectedCity) loadDoctors(selectedSpecialty, selectedState, selectedCity); setSelectedDoctor(''); }, [selectedSpecialty, selectedState, selectedCity, loadDoctors]);
  useEffect(() => { setSelectedDate(''); }, [selectedDoctor]);
  useEffect(() => { if (selectedDoctor && selectedDate) loadAvailableTimeSlots(selectedDoctor, selectedDate); setSelectedTime(''); }, [selectedDoctor, selectedDate, loadAvailableTimeSlots]);
