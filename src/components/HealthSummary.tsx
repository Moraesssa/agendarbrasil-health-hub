...
import { Activity, Heart, Thermometer, Weight, LucideIcon } from "lucide-react";
...
export interface HealthMetric {
 ...
 value: string | number;
 unit?: string;
 status: "normal" | "ideal" | "atenção" | "crítico";
 icon: LucideIcon;
 color: string;
}
...
const HealthSummary = ({ healthMetrics, healthScore }: HealthSummaryProps) => {
     ...
     <CardContent className="space-y-3 sm:space-y-4 px-4 sm:px-6">
         ...
         >
           ...
           <div className="text-right flex-shrink-0 ml-2">
             <p className="font-semibold text-gray-900 text-sm">
               {metric.value} {metric.unit && <span className="text-xs text-gray-500">{metric.unit}</span>}
             </p>
       ...
       <div className="mt-4 sm:mt-6 p-4 rounded-xl bg-gradient-to-br from-blue-50 to-green-50 border border-blue-100">
         <div className="flex items-center justify-between mb-3">
           ...
           <span className="text-xl sm:text-2xl font-bold text-blue-600">{healthScore.value}%</span>
         </div>
         <Progress value={healthScore.value} className="h-2 mb-2" />
