-- SQL script to create the prescription table and fix relationships

-- Step 1: Create the DocumentosDigitais table (as per schema.sql)
...
-- Step 2: Add doctor_id and patient_id columns to DocumentosDigitais
...
-- Step 3: Add foreign key constraints for doctor_id and patient_id
...
-- Step 4: Rename the table to 'medical_prescriptions' to match frontend expectations
ALTER TABLE "DocumentosDigitais" RENAME TO "medical_prescriptions";

-- Step 5: Add the 'is_active' column (as seen in frontend error logs)
ALTER TABLE "medical_prescriptions" ADD COLUMN IF NOT EXISTS "is_active" BOOLEAN DEFAULT TRUE;

-- Step 6: Add the 'prescribed_date' column (as seen in frontend error logs)
